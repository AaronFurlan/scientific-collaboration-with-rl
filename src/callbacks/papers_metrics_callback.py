...
class PapersMetricsCallback(DefaultCallbacks):
    ...
    def on_episode_step(
        ...
        **kwargs,
        ...
        # New effort metrics
        ud["_effort_total"] = info.get("effort_total_count", ud["_effort_total"])
        ud["_effort_invalid"] = info.get("effort_invalid_count", ud["_effort_invalid"])
        ud["_effort_valid"] = info.get("effort_valid_count", ud["_effort_valid"])

        ea = info.get("effort_analysis")
        if isinstance(ea, dict):
            if ea.get("effort_idx", 0) > 0:
                ud["_chosen_prestige"].append(float(ea.get("chosen_prestige", 0.0)))
                ud["_max_prestige"].append(float(ea.get("max_prestige", 0.0)))
                ud["_chose_max_prestige"].append(int(ea.get("chose_max_prestige", 0)))
                ud["_chosen_rem_time"].append(float(ea.get("chosen_remaining_time", 0.0)))
                ud["_min_rem_time"].append(float(ea.get("min_remaining_time", 0.0)))
                ud["_chose_most_urgent"].append(int(ea.get("chose_most_urgent", 0)))
    ...
    def on_episode_end(
        ...
        **kwargs,
        ...
        metrics = {
            ...
            "effort/invalid_count": int(ud.get("_effort_invalid", 0)),
            "effort/valid_count": int(ud.get("_effort_valid", 0)),
            "effort/invalid_rate": float(ud.get("_effort_invalid", 0) / ud.get("_effort_total", 1)) if ud.get("_effort_total", 0) > 0 else 0.0,
            # Decision quality: Prestige
            "effort/chosen_prestige": float(np.mean(ud.get("_chosen_prestige", []))) if ud.get("_chosen_prestige") else 0.0,
            "effort/max_prestige": float(np.mean(ud.get("_max_prestige", []))) if ud.get("_max_prestige") else 0.0,
            "effort/chose_max_prestige": float(np.mean(ud.get("_chose_max_prestige", []))) if ud.get("_chose_max_prestige") else 0.0,
            # Decision quality: Deadlines
            "effort/chosen_remaining_time": float(np.mean(ud.get("_chosen_rem_time", []))) if ud.get("_chosen_rem_time") else 0.0,
            "effort/min_remaining_time": float(np.mean(ud.get("_min_rem_time", []))) if ud.get("_min_rem_time") else 0.0,
            "effort/chose_most_urgent": float(np.mean(ud.get("_chose_most_urgent", []))) if ud.get("_chose_most_urgent") else 0.0,
        }
        
        # RLlib prefixes custom_metrics with 'custom_metrics/' and appends '_mean', '_min', '_max'.
        # For our CSV logging in train_rl_agent.py, we also want to ensure they are present.
